...
CC8Emulator::CC8Emulator(std::string filename, SDL_Window *gWindow, SDL_Renderer *gRenderer)
...
void CC8Emulator::StartProcessing()
{
    mainloop();
}

void CC8Emulator::StopProcessing()
{
    //TODO
}

/*reads filename to open from cin and copies a program into ram starting at 0x200*/
void CC8Emulator::read_input_file(std::string filename) {
...
void CC8Emulator::dump_ram(std::string filename) {
...
std::ostream& CC8Emulator::dump_emu_core(std::ostream& os) {
...
std::bitset<120> CC8Emulator::givesprite(char c) {
...
uint16_t CC8Emulator::givecharaddress(uint8_t c)
{
...
void CC8Emulator::startbeep(int channel)
{
    //start playing a continuous, looping sound
    Mix_Resume(channel);
}

void CC8Emulator::stopbeep(int channel)
{
    //stop playing the continuous sound
    Mix_Pause(channel);
}

void CC8Emulator::mainloop() {
    //Main loop flag
    bool quit = false;
...
void CC8Emulator::execute(uint16_t cmd, const std::vector<SDL_Keycode> &keypressed) {
